...
ErrorOr<OpenSSL_BN> unsigned_big_integer_to_openssl_bignum(UnsignedBigInteger const& integer)
{
    ...
    auto buf = TRY(ByteBuffer::create_uninitialized(integer.byte_length(), ByteBuffer::EraseBufferOnFree::Yes));
...
ErrorOr<UnsignedBigInteger> openssl_bignum_to_unsigned_big_integer(OpenSSL_BN const& bn)
{
    ...
    auto buf = TRY(ByteBuffer::create_uninitialized(size, ByteBuffer::EraseBufferOnFree::Yes));
...
ErrorOr<ByteBuffer> get_byte_buffer_param_from_key(OpenSSL_PKEY& key, char const* key_name)
{
    ...
    auto buffer = TRY(ByteBuffer::create_uninitialized(size, ByteBuffer::EraseBufferOnFree::Yes));
