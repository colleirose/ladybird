        ...
        for_each_chunk([&](auto chunk) {
            ...
            if constexpr (use_mmap) {
                ...
                if (!status) {
                    ...
                    // fallback to erasing the memory if we can't free it
                    // BumpAllocator is currently not being used for anything particularly sensitive, but this is a cheap operation and should be fine to do
        ...
        if (!new_chunk) {
            if constexpr (use_mmap) {
                ...
                if (new_chunk == MAP_FAILED) {
                    warnln("mmap failed in BumpAllocator.h with error {}", Error::from_errno(errno));
                    return false;
                }
#endif
            } else {
